...
class ItemPropertyInjectMultiplyHandler<T : Number?> : ItemPropertyInjectHandler<T>() {
    override fun doHandle(source: T, value: T): T {
        if (source is Number && value is Number) {
            return NumberHandler.doHandles(source, value, ItemPropertyInjectAction.MULTIPLY)
        }
        return source
    }
}
