...
open class ConiumRarityTemplate(private val rarity: Rarity, name: String) : ConiumItemTemplate(name = name) {
    companion object {
        @JvmStatic
        fun create(element: JsonElement, registryLookup: RegistryWrapper.WrapperLookup, name: String = Item.RARITY): ConiumRarityTemplate = ConiumRarityTemplate(createRarity(element.asString), name)

        @JvmStatic
        fun createBedrock(element: JsonElement, registryLookup: RegistryWrapper.WrapperLookup): ConiumRarityTemplate = create(element, registryLookup, BedrockItem.RARITY)
    }
