 ...
 *
 * The registries are confirmed in constructing, keys cannot dynamically change in the registrable NBT lifecycle.
 *
...
class RegistrableNbt(
    ...
    fun writeTo(nbt: NbtCompound, registries: RegistryWrapper.WrapperLookup) {
        // Using registries to serialize data, only registered data can be write, doesn't write others data anymore.
    ...
    fun readFrom(nbt: NbtCompound, registries: RegistryWrapper.WrapperLookup) {
        // Using registries to deserialize data, only registered data can be read, doesn't read others data anymore.
        for ((name, serializer) in this.registries) {
