...
public class ItemPropertyInjectDivideHandler<T extends Number> extends ItemPropertyInjectHandler<T> {
    @Override
    public T doHandle(T source, T value) {
        if (source instanceof Number sourceNumber && value instanceof Number valueNumber) {
            return NumberHandler.doHandles(sourceNumber, valueNumber, ItemPropertyInjectAction.DIVIDE);
        }
        return source;
    }
}
