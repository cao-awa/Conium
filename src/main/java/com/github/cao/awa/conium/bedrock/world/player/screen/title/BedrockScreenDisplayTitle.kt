...
class BedrockScreenDisplayTitle(private var title: String, private var subtitle: String, private var active: Boolean = false) {
    fun setTitle(title: String, properties: BedrockScriptAnonymousObjectMap) {
        MinecraftClient.getInstance().inGameHud.let { inGameHud ->
            ...
            ).arising(Any(), properties.toDynamicArgs()) { _, fadeInTicks, stayTicks, fadeOutTicks ->
    ...
    fun updateSubtitle(title: String) {
        MinecraftClient.getInstance().inGameHud.setSubtitle(Text.of(title))
    }
}
